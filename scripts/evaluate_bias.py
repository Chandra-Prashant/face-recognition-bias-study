...
# Settings based on Methodology
MODELS = ["FaceNet", "ArcFace", "VGG-Face"]
# Use absolute path to avoid Mac "Working Directory" confusion
...
def calculate_similarity(v1, v2):
    """Quantifying performance using Cosine Similarity."""
...
for race in groups:
    ...
    image_list = os.listdir(race_path)[:50]
...
# 4. Save Results for ANOVA/T-Tests
if results_data:
    df = pd.DataFrame(results_data)
    df.to_csv(RESULTS_FILE, index=False)
    print(f"\n🚀 SUCCESS: {len(results_data)} records saved to {os.path.abspath(RESULTS_FILE)}")
else:
    print("\n❌ FAILED: No data points were collected. Check the logs above.")